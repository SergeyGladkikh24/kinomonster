	...
	function register()
	{
		...
		{	
			...
			{	
				...
				{					
					$data['auth_message'] = 'Вы успешно зарегистрировались.'.anchor(site_url($this->dx_auth->login_uri), ' ');
				}
		...
		{
			$data['auth_message'] = 'Регистрация отклонена.';
			$this->load->view('templates/header', $this->data);
		...
		{
			$data['auth_message'] = 'Прежде чем регистрироваться, вам необходимо выйти из системы.';
			$this->load->view('templates/header', $this->data);
			$this->load->view($this->dx_auth->logged_in_view, $data);
			$this->load->view('templates/footer');
		}
